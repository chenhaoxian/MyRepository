...
Index.prototype.init = function() {
	if(cookieUtil.get('user_cookies') == null){
		var food_tbody = document.getElementById("food_table_data");
		var cart_tbody = document.getElementById("shoppingcart_table_data");
		
		var food_json = JSON.parse(cookieUtil.get('food_data')); 
		var cart_json = JSON.parse(cookieUtil.get('cart_data'));
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				if(result.indexOf("#") != -1){
					...
					//cookieUtil.set('cart_data',cart_data,setCookieDate(1));
				}else{
					food_data = result;
				}
